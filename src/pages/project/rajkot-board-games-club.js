...
const ProjectPage = ({ content }) => {
	...
	return (
		<div>
			<Head>
				<title>Rajkot Board Games Club</title>
				<meta
					name="description"
					content="Rajkot Board Games Club project overview"
				/>
				<meta name="viewport" content="width=device-width, initial-scale=1" />
				<link rel="icon" href="/line3.png" />
			</Head>
			<div className="bg-slate-50 pb-36">
				<div className="h-96 w-screen bg-center bg-auto bg-no-repeat  bg-[url('https://images.pexels.com/photos/278918/pexels-photo-278918.jpeg')]"></div>
				<div className="  mx-auto px-6 md:px-12 xl:px-32">
					<div className=" flex flex-col -mt-40 block rounded-lg  px-6 py-12 md:py-16 md:px-12 bg-white min-h-screen  max-w-3xl mx-auto shadow-2xl">
						<h1 className="text-left text-4xl text-slate-900 font-bold">
							Rajkot Board Games Club
						</h1>
						<div className="my-8 flex flex-wrap gap-2">
							{projects
								.find((p) => p.title === "Rajkot Board Games Club")
...
const LiveDemoButton = ({ children }) => {
	return (
		<Link href={"https://rbgc-website.netlify.app/"} target="_blank">
...
export async function getStaticProps() {
	const filePath = path.join(
		process.cwd(),
		"/src/data/rajkot-board-games-club.md",
	);
